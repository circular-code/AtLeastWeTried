...
export const useGameStore = defineStore('game', {
    ...
    getters: {
        ...
        overlayEntry: (state) => (controllableId) => {
            ...
            return buildOverlayEntry(controllableId, state.galaxy, new Map(Object.entries(overlayById)), uiStore.selectedControllableId);
        },
        selectionEntry: (state) => (selection) => {
            return buildClickedUnitEntry(selection, state.galaxy, new Map(Object.entries(aggregateOverlayState(state.overlayBySessionId))));
        },
        scannerModeFor: (state) => (controllableId) => {
            if (!controllableId) {
                return 'off';
            }
        ...
        scannerTargetFor: (state) => (controllableId) => {
            if (!controllableId) {
                return undefined;
            }
            const overlayState = objectValue(resolveOverlayByControllableId(state.overlayBySessionId, controllableId)) ?? {};
        ...
        scannerWidthFor: (state) => (controllableId) => {
            if (!controllableId) {
                return 90;
            }
            const overlayState = objectValue(resolveOverlayByControllableId(state.overlayBySessionId, controllableId)) ?? {};
        ...
        scannerWidthMinimumFor: (state) => (controllableId) => {
            if (!controllableId) {
                return 5;
            }
            const overlayState = objectValue(resolveOverlayByControllableId(state.overlayBySessionId, controllableId)) ?? {};
        ...
        scannerWidthMaximumFor: (state) => (controllableId) => {
            if (!controllableId) {
                return 90;
            }
            const overlayState = objectValue(resolveOverlayByControllableId(state.overlayBySessionId, controllableId)) ?? {};
        ...
        scannerLengthFor: (state) => (controllableId) => {
            if (!controllableId) {
                return 200;
            }
            const overlayState = objectValue(resolveOverlayByControllableId(state.overlayBySessionId, controllableId)) ?? {};
        ...
        scannerLengthMinimumFor: (state) => (controllableId) => {
            if (!controllableId) {
                return 1;
            }
            const overlayState = objectValue(resolveOverlayByControllableId(state.overlayBySessionId, controllableId)) ?? {};
        ...
        scannerLengthMaximumFor: (state) => (controllableId) => {
            if (!controllableId) {
                return 200;
            }
            const overlayState = objectValue(resolveOverlayByControllableId(state.overlayBySessionId, controllableId)) ?? {};
        ...
        scannerRequestedModeFor: (state) => (controllableId) => {
            if (!controllableId) {
                return 'off';
            }
            const overlayState = objectValue(resolveOverlayByControllableId(state.overlayBySessionId, controllableId)) ?? {};
            ...
            if (mode === '360' || mode === 'full')
                return '360';
            if (mode === 'forward')
                return 'forward';
            if (mode === 'sweep')
                return 'sweep';
            if (mode === 'targeted' || mode === 'target')
                return 'targeted';
            return 'off';
        },
        tacticalModeFor: (state) => (controllableId) => {
            if (!controllableId) {
                return 'off';
            }
            const overlayState = objectValue(resolveOverlayByControllableId(state.overlayBySessionId, controllableId)) ?? {};
        ...
        thrustPercentageFor: (state) => (controllableId) => {
            if (!controllableId) {
                return 1;
            }
            const overlayState = objectValue(resolveOverlayByControllableId(state.overlayBySessionId, controllableId)) ?? {};
        ...
        recentActivity: (state) => (lifetimeMs) => {
            const now = Date.now();
    ...
    actions: {
        ...
        applyOwnerDelta(message) {
            ...
            nextOverlayBySessionId.set(message.playerSessionId, new Map(Object.entries(next)));
            this.overlayBySessionId = nextOverlayBySessionId;
        },
        syncAttachedPlayerSessions(playerSessions) {
        ...
        recordTeamScoreActivities(previousGalaxy, nextGalaxy) {
        ...
        clearNavigationOverlay(controllableId) {
            const nextOverlayBySessionId = new Map(this.overlayBySessionId);
            let updated = false;
        ...
        getControllableLabel(controllableId) {
...
function applyWorldDeltaToSnapshot(current, message) {
...
function createUnitFromChanges(unitId, changes) {
    const unit = {
        unitId,
        clusterId: 1,
        kind: 'unknown',
...
function applyUnitChanges(unit, changes) {
...
function dedupeUnitsById(units) {
...
function pruneTransientHiddenUnits(units) {
...
function aggregateOverlayState(overlayBySessionId) {
    const aggregate = {};
...
function resolveOverlayByControllableId(overlayBySessionId, controllableId) {
    for (const overlayById of overlayBySessionId.values()) {
...
function pruneOverlaySessions(overlayBySessionId, playerSessions) {
    const attachedSessionIds = new Set(playerSessions.map((player) => player.playerSessionId));
    return new Map(Array.from(overlayBySessionId.entries()).filter(([playerSessionId]) => attachedSessionIds.has(playerSessionId)));
}
function consumePendingCommand(store, commandId) {
...
function applyKnownUnitIntelMetric(unit, key, value) {
    if (value === undefined) {
        return;
    }
    unit[key] = mergeKnownUnitIntelMetric(unit[key], value);
}
function mergeKnownUnitIntelMetric(currentValue, incomingValue) {
...
function isOverlayCommandable(overlayState) {
    if (hasRecordKey(overlayState, 'isCommandable')) {
        return booleanValue(overlayState.isCommandable, false);
    }
    return true;
}
function buildOverlayEntry(controllableId, snapshot, overlayById, selectedControllableId) {
...
function buildClickedUnitEntry(selection, snapshot, overlayById) {
...
function buildDetailGroups(unit, kindHint) {
    const groups = [];
...
function deriveScannerWidth(scannerState) {
    if (!scannerState) {
        return 90;
    }
    return numberValue(scannerState.requestedWidth, numberValue(scannerState.targetWidth, numberValue(scannerState.currentWidth, deriveScannerMaximumWidth(scannerState))));
}
function deriveScannerMinimumWidth(scannerState) {
    if (!scannerState) {
        return 5;
    }
    return numberValue(scannerState.minimumWidth, 5);
}
function deriveScannerMaximumWidth(scannerState) {
    if (!scannerState) {
        return 90;
    }
    return numberValue(scannerState.maximumWidth, 90);
}
function deriveScannerLength(scannerState) {
    if (!scannerState) {
        return 200;
    }
    return numberValue(scannerState.requestedLength, numberValue(scannerState.targetLength, numberValue(scannerState.currentLength, deriveScannerMaximumLength(scannerState))));
}
function deriveScannerMinimumLength(scannerState) {
    if (!scannerState) {
        return 1;
    }
    return numberValue(scannerState.minimumLength, 1);
}
function deriveScannerMaximumLength(scannerState) {
    if (!scannerState) {
        return 200;
    }
    return numberValue(scannerState.maximumLength, 200);
}
function resolveScannerState(scannerValue) {
