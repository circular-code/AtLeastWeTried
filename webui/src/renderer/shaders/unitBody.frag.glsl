...
float maskShipFrontTriangle(vec2 point) {
  vec2 markerPoint = point - vec2(0.54, 0.0);
  float baseToTip = 0.20;
  float halfBaseHeight = 0.25;
  float edgeSoftness = 0.018;

  // Triangle spans x in [-0.14, 0.14], with the tip at +x.
  float minX = -0.14;
  float maxX = 0.14;
  float localWidth = max(0.0, (maxX - markerPoint.x) * (halfBaseHeight / baseToTip));

  float insideX =
    smoothstep(minX - edgeSoftness, minX + edgeSoftness, markerPoint.x) *
    (1.0 - smoothstep(maxX - edgeSoftness, maxX + edgeSoftness, markerPoint.x));
  float insideY = 1.0 - smoothstep(localWidth - edgeSoftness, localWidth + edgeSoftness, abs(markerPoint.y));
...
void main() {
  ...
  // kinds 7-9: classic-ship, modern-ship, ship
  } else if (kind < 9.5) {
    ...
    float directionMask = maskShipFrontTriangle(point) * rimMask;
    vec3 hullColor = mix(color * 0.68, color * 1.08 + vec3(0.08, 0.1, 0.14), smoothstep(-0.75, 0.45, point.x));

    baseMask = hullMask;
    glowMask = maskCircle(point, 0.96) * 0.12;
