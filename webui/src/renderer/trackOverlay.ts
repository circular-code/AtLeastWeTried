...
export type TrackedTargetVisual = {
  color: string;
  marker: THREE.Group;
  line: THREE.Line;
  edgeIndicator: THREE.LineLoop;
};

export function createTrackedTargetVisual(color: string): TrackedTargetVisual {
  ...
  const edgeIndicator = new THREE.LineLoop(
    new THREE.BufferGeometry().setFromPoints(createRingPoints(20, 1)),
    new THREE.LineBasicMaterial({
      color,
      transparent: true,
...
export function disposeTrackedTargetVisual(visual: TrackedTargetVisual) {
  ...
  (visual.edgeIndicator.material as THREE.Material).dispose();
}
